...
class Extension extends BaseExtension
{
    ...
    public function buildForm() {
    	$form = $this->app['form.factory']->createBuilder('form', array())
            ...
            ->add('subject', 'text', array(  //honeypot
                'required' => false,
                'constraints' => array(new Assert\NotBlank())
            ))
    ...
    public function renderForm($view = NULL) {
        ...
        $url = parse_url($this->app['resources']->getUrl('currenturl'), PHP_URL_PATH);

        switch ($url) {
            case '/need-help/in-crises':
                $subject_value = 'In Crises, Need Help!!!';
                break;
            case '/need-help/want-to-volunteer':
                $subject_value = 'I Want to Volunteer!!!';
                break;
            case '/need-help/information-about-event':
                $subject_value = 'I Need Information!!!';
                break;
            default:
                $subject_value = 'I Need Help!!!';
                break;
        }
    ...
    public function handleForm(Request $request) {
	    ...
	    if ($form->isValid()) {
            ...
            if($data['subject'] == 'In Crises, Need Help!!!')
            {
                $to = $this->config['toCrises'];
            }
            else
            {
                $to = $this->config['to'];
            ...
            $subject = sprintf("[%s] %s", $this->app['config']->get('general/sitename'), $data['subject']);
